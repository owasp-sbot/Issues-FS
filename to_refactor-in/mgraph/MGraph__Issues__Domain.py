...
class MGraph__Issues__Domain(Type_Safe):                                         # Graph domain operations
    data : Schema__MGraph__Issues__Data = None                                   # Underlying data

    # Indexes (computed in memory, not persisted separately)
    index_by_label  : Dict[str, str]       = None                                # label → node_id
    index_by_path   : Dict[str, str]       = None                                # path → node_id
    index_by_parent : Dict[str, List[str]] = None                                # parent_id → [child_ids]

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if self.data is None:
            self.data = Schema__MGraph__Issues__Data()
